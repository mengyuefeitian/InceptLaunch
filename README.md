...
- **文件夹缩放动画** —— 弹层从网格瓦片位置放大打开，关闭时缩回原位（过渡中保持 3×3 形态）。
- **迷你图标直启** —— 放大后的 2×2 大文件夹上，可不打开弹层，直接点击内部迷你图标启动应用。
- **启动不卡顿** —— 点击 app 后先退大屏再异步启动，避免 2–5 秒假死。
- **首击可用** —— 打开大屏后的第一次点击（启动 / 开文件夹 / 点空白退出）即可生效。
- **交互更稳** —— 文件夹开合不再卡在半开状态；关闭大屏时把焦点还给之前的前台应用。
